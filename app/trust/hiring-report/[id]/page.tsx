...
export default async function HiringReportPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [
    ...
    supabase
      ...
      .select("id,replay_summary,created_at,generated_by")
      .eq("subject_type", "interview_session")
      .eq("subject_id", id)
      .order("created_at", { ascending: false })
      .limit(1)
    ...
    <main className="min-h-screen bg-[#04070c] px-6 py-12 text-white md:px-8">
      <div className="mx-auto max-w-6xl">
        ...
        <section className="mt-8 rounded-lg border border-zinc-800 bg-zinc-950 p-5">
          <h2 className="text-xl font-semibold">Replay Memory</h2>
          {latestReplay ? (
            <div className="mt-5 rounded-lg border border-zinc-800 bg-black p-4">
              <p className="text-sm leading-6 text-zinc-300">
                {latestReplay.replay_summary ?? "Replay context is available for this interview workflow."}
              </p>
              <p className="mt-3 text-xs text-zinc-600">
                Generated by {latestReplay.generated_by ?? "system"}
              </p>
            </div>
          ) : (
            <p className="mt-5 rounded-lg border border-zinc-800 bg-black p-4 text-sm text-zinc-500">
              No replay snapshot has been recorded yet.
            </p>
          )}
        </section>
