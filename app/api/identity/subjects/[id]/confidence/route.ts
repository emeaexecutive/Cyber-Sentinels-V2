...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try { const context = await resolveIdentityEnterprise(request); const { id } = await params; if (!isUuid(id)) throw new Error("A valid subject id is required."); await identityRepository().assertSubject(context.enterpriseId, id); return identitySuccess({ subjectId: id, confidence: await identityRepository().subjectConfidence(context.enterpriseId, id) }); }
