...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!details) return Response.json({ schemaVersion: 1, ok: false, code: "NOT_FOUND", error: "Verification was not found in this enterprise." }, { status: 404 });
    return identitySuccess({ verification: details });
