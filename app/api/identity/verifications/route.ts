...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (Number(request.headers.get("content-length") ?? 0) > 64_000) return Response.json({ ok: false, code: "PAYLOAD_TOO_LARGE", error: "Payload is too large." }, { status: 413 });
    ...
    const result = await orchestrateIdentityVerification({ repository: identityRepository(), adapters: buildIdentityAdapters(hopaeStarter), enterpriseId: context.enterpriseId, subjectId: body.subjectId, requestedSignals, purpose, idempotencyKey, actorId: context.user.id, signalInputs });
    return identitySuccess({ verification: result.details, replayed: result.replayed, reasonCode: result.reasonCode }, result.replayed ? 200 : 202);
  } catch (error) {
    if (error instanceof Error && "status" in error) return Response.json({ schemaVersion: 1, ok: false, code: (error as Error & { code?: string }).code, error: error.message }, { status: Number((error as Error & { status?: number }).status ?? 409) });
