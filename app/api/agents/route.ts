  ...
  await bestEffort("AI agent receipt bundle insert", async () => {
    await createReceiptBundle(supabase, {
      ...
      evidenceSnapshot: {
        ...
        trust_score_confidence: "Uncalibrated until benchmark validation",
        trust_score_evidence: ["Registry state", "permission scope", "owner mapping", "governance status"],
        trust_score_limitations: ["Not a real-ML verdict", "Requires human governance review"],
        operational_identity: {
          agent_id: agent.id,
          human_authority: user.id,
          owner_mapping: agent.owner_email,
          authorization_lineage: insert.operational_scope,
          least_privilege_scope: agent.permissions,
          runtime_risk: text(payload.risk_level, "pending assessment"),
          blast_radius: agent.permissions,
          kill_switch_status: agent.status === "blocked" ? "engaged" : "available",
