...
export async function GET(
  ...
  try {
    ...
    const events = await new ReplayService(
      createReplayRepository(auth.supabase),
    ).events(auth.enterpriseId, entityId, replaySearch(request));
    return trustGraphResponse({ ok: true, entityId, events }, 200, correlationId);
  } catch (error) {
