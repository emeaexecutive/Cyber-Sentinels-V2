...
export async function GET(
  ...
  try {
    ...
    const history = await new EvidenceGraphService(
      createEvidenceRepository(auth.supabase),
    ).getHistory(auth.enterpriseId, identityId, trustIntelligenceLimit(request));
    return trustIntelligenceResponse({ ok: true, history }, 200, correlationId);
  } catch (error) {
