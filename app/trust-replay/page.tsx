...
export default async function TrustReplayPage({ searchParams }: TrustReplayPageProps) {
  ...
  const replayValidationRows = [
    ...
    ["Human owner", snapshot.canonicalMemory.humanOwners.join(", ") || "No human owner retained in this replay window"],
    ["Permission scope", snapshot.canonicalMemory.permissionScopes.join(", ") || "No permission scope retained in this replay window"],
    ["Authorization lineage", snapshot.canonicalMemory.authorizationLineage],
    ["Trust score source", snapshot.canonicalMemory.trustScoreSources.join(", ") || "No trust-score source retained in this replay window"],
    ["Runtime signals", snapshot.canonicalMemory.runtimeSignals.slice(0, 4).join(", ") || "No runtime signal retained in this replay window"],
    ["Governance action", snapshot.canonicalMemory.governanceActions.join(", ") || snapshot.canonicalMemory.governanceState],
    ["Decision reason", snapshot.canonicalMemory.decisionReasons.join(", ") || "No allow/block/escalate reason retained in this replay window"],
