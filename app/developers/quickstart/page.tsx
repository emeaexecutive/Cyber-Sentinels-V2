...
export default function DeveloperQuickstartPage() {
  const base = "https://your-preview.example";
  return <main className="min-h-screen bg-[#04070c] px-6 py-14 text-white"><div className="mx-auto max-w-5xl">
    ...
    <ol className="mt-10 grid gap-8">
      <li><h2 className="text-xl font-semibold">1. Create a scoped API key</h2><p className="mt-2 text-sm text-zinc-400">Create the key in the portal, copy the secret once, and keep it in a server-side secret store.</p></li>
...
await cs.agents.submitProof(gamma.agent_id, await signChallenge(challenge, gamma.manifest_context.enterprise_id, credential.credential_id, keys.privateKey));`}</pre></li>
      <li><h2 className="text-xl font-semibold">7–8. Request an exact action and handle every decision</h2><pre className={`${code} mt-3`}>{`const result = await cs.trust.authorize({ operational_entity_id: gamma.operational_entity_id, action: { type: "read_repository", target: "repository:a", purpose: "deployment_evidence_review", environment: "staging" }, idempotency_key: "gamma-action-001" });
if (result.decision !== "ALLOW") { /* stop or enter human review */ }
// authorize() never executes customer code.`}</pre></li>
      <li><h2 className="text-xl font-semibold">9–10. Retrieve transaction, Replay and receipt</h2><pre className={`${code} mt-3`}>{`await cs.trust.getTransaction(result.transaction_id);
await cs.trust.getReplay(result.transaction_id);
await cs.trust.getReceipt(result.transaction_id);`}</pre></li>
    </ol>
...
curl -X POST "$CS_BASE/api/v1/trust/transactions/$TRANSACTION_ID/outcomes" -H "Authorization: Bearer $CS_KEY" -H "Content-Type: application/json" --data @outcome.json`}</pre></section>
    <p className="mt-8 text-sm text-zinc-500">Use the OpenAPI document as the contract source of truth. 429 responses include Retry-After. All errors use <code>{`{"error":{"code","message","correlation_id"}}`}</code>.</p>
  </div></main>;
}
