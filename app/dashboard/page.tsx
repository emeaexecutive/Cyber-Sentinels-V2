...
export default async function DashboardPage() {
  ...
  const [flags, reviews, workflows, threats, integrity, receipts] = await Promise.all([
    ...
    supabase.from("interview_risk_events").select("*", { count: "exact", head: true }).ilike("signal_type", "%injection%"),
  ...
  const metrics = [
    ...
    ["Threat Activity", threats.count ?? 0, Activity],
    ["Session Integrity", integrity.count ?? 0, ScanSearch],
    ["Verification Receipts", receipts.count ?? 0, ReceiptText],
  ] as const;
      ...
      <section className="mx-auto max-w-7xl">
        ...
        <section className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {metrics.map(([title, value, Icon]) => (
            <div key={title} className="rounded-2xl border border-sentinel-line bg-white/[0.04] p-5">
        ...
        <section className="mt-8 rounded-2xl border border-sentinel-line bg-sentinel-panel/80 p-5">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              <h2 className="text-xl font-semibold">Pilot review path</h2>
              <p className="mt-2 text-sm text-sentinel-muted">Three operator surfaces cover the complete workflow.</p>
            </div>
