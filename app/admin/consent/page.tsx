...
import { getConsentDefaultEnterpriseId } from "@/src/lib/config/consent-config";
...
export default async function ConsentAdminPage({searchParams}:{searchParams:Promise<{enterpriseId?:string}>}) {
  ...
  const requestedEnterpriseId = (await searchParams).enterpriseId;
  const enterpriseId = requestedEnterpriseId ?? (() => {
    try {
      return getConsentDefaultEnterpriseId();
    } catch {
      return "";
    }
  })();
