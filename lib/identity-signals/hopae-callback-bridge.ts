...
export async function bridgeHopaeCallbackToIdentity(result: Record<string, unknown>) {
  ...
  const evidenceUpdate = await database.from("identity_signal_evidence").update({ outcome, confidence, server_verified: accepted, source_digest: normalized.data?.source_digest ?? null, reason_codes: [accepted ? "HOPAE_SIGNED_CALLBACK_EVIDENCE_ACCEPTED" : "HOPAE_EVIDENCE_QUALITY_NOT_ACCEPTED"], limitations: Array.isArray(quality.limitations) ? quality.limitations : [], expires_at: normalized.data?.expires_at ?? null, observed_at: new Date().toISOString() }).eq("provider_transaction_id", transaction.data.id);
  ...
  const evidenceRows = await database.from("identity_signal_evidence").select("signal_type,provider_id,outcome,confidence,server_verified,source_digest,reason_codes,limitations,attributes,observed_at,expires_at").eq("verification_request_id", transaction.data.verification_request_id);
  if (evidenceRows.error) throw evidenceRows.error;
  const drafts = (evidenceRows.data ?? []).map((row) => ({ signalType: row.signal_type, providerId: row.provider_id, outcome: row.outcome, confidence: Number(row.confidence), serverVerified: row.server_verified, sourceDigest: row.source_digest, reasonCodes: row.reason_codes, limitations: row.limitations, attributes: row.attributes, observedAt: row.observed_at, expiresAt: row.expires_at })) as SignalEvidenceDraft[];
