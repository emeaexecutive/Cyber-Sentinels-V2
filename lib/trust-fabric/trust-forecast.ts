...
function controlsFor(conditions: TrustConditionInput[], changes: string[], findings: string[]): TrustForecastControlRecommendation[] {
  ...
  if (signals.has("MODEL_CONTROLLED_SECURITY_BOUNDARY") || signals.has("DESTINATION_CHANGED")) add("PIN_DESTINATION", "pin_destination", "PAUSE", 1, "Bind the destination outside model control before qualification.", ["MODEL_CONTROLLED_SECURITY_BOUNDARY", "DESTINATION_CHANGED"]);
  if (signals.has("STALE_AUTHORITY_STILL_ACTIVE")) add("REVOKE_STALE_CREDENTIAL", "revoke_stale_credential", "PAUSE", 1, "Revoke stale credentials and independently confirm downstream rejection.", ["STALE_AUTHORITY_STILL_ACTIVE"]);
...
export function evaluateTrustForecast(input: TrustForecastEvaluationInput): TrustForecast {
  ...
  const authorityPressureSignals = new Set(["AUTHORITY_CHANGED", "PRIVILEGE_INCREASED", "AUTHORITY_SOURCE_CHANGED", "DELEGATED_PRINCIPAL_LOST", "RUNTIME_SCOPE_WIDENED", "STALE_AUTHORITY_STILL_ACTIVE"]);
...
export function createPreActionTrustForecastInput(context: PreActionTrustContext): TrustForecastEvaluationInput {
  ...
  const modelControlledBoundary = findings.includes("MODEL_CONTROLLED_SECURITY_BOUNDARY");
  const conditions: TrustConditionInput[] = [
    ...
    common("TOOL_PARAMETER_PROVENANCE", context.parameterProvenanceReference ? "STABLE" : "UNKNOWN", context.parameterProvenanceReference ? "Tool parameters retain a digest or provenance reference." : "Tool parameter provenance is unavailable.", { confidence: context.parameterProvenanceReference ? 0.9 : 0.25, evidenceReferences: context.parameterProvenanceReference ? [context.parameterProvenanceReference] : [], knownLimitations: context.parameterProvenanceReference ? [] : ["TOOL_PARAMETER_PROVENANCE_UNAVAILABLE"] }),
    ...
    common("DESTINATION_EXPOSURE", modelControlledBoundary ? "ELEVATED" : "STABLE", `Destination binding is ${context.destinationReference}.`, { trend: modelControlledBoundary ? "DETERIORATING" : "UNCHANGED", materiality: "CRITICAL", signals: modelControlledBoundary ? ["MODEL_CONTROLLED_SECURITY_BOUNDARY"] : [] }),
    common("RUNTIME_ASSURANCE", "STABLE", `Runtime state is ${context.runtimeReference}.`),
    ...
    common("AUTHORIZATION_PROPAGATION", staleAuthority ? "SEVERE" : "STABLE", staleAuthority ? "Revoked or downgraded authority remains active downstream." : "No stale downstream authority was observed.", { trend: staleAuthority ? "RAPIDLY_DETERIORATING" : "UNCHANGED", materiality: "CRITICAL", signals: staleAuthority ? ["STALE_AUTHORITY_STILL_ACTIVE"] : [] }),
    common("STALE_AUTHORITY_RISK", staleAuthority ? "SEVERE" : "STABLE", staleAuthority ? "Old authority still works downstream." : "No active stale-authority evidence was observed.", { trend: staleAuthority ? "RAPIDLY_DETERIORATING" : "UNCHANGED", materiality: "CRITICAL", signals: staleAuthority ? ["STALE_AUTHORITY_STILL_ACTIVE"] : [] }),
  ];
  return deepFreeze({
