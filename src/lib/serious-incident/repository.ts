...
export function seriousIncidentRepository() {
  ...
  return {
    ...
    async reviewerAssignments(enterpriseId: string, incidentId: string, actorId: string) {
      const result = await db.from("incident_responsibility_roles").select("role_type,authority_reference").eq("enterprise_id", enterpriseId).eq("incident_id", incidentId).eq("party_reference", actorId).is("superseded_at", null);
      if (result.error) failure("read reviewer assignments", result.error);
      return (result.data ?? []).map((row) => ({ role: String(row.role_type) as ReviewerRole, authorityReference: row.authority_reference ? String(row.authority_reference) : null }));
    },
    ...
    async canonicalReference(enterpriseId: string, table: "execution_context_declarations" | "environment_attestations" | "scope_authorization_leases" | "scope_continuity_decisions", id: string) {
      const result = await db.from(table).select("id").eq("enterprise_id", enterpriseId).eq("id", id).maybeSingle();
      if (result.error) failure(`read canonical ${table} reference`, result.error); return result.data;
    },
    async approvedPolicy(enterpriseId: string, policyVersionId: string) {
      const now = new Date().toISOString();
      const result = await db.from("trust_policy_versions").select("policy_version_id").eq("enterprise_id", enterpriseId).eq("policy_version_id", policyVersionId).eq("active", true).lte("valid_from", now).or(`valid_until.is.null,valid_until.gt.${now}`).maybeSingle();
      if (result.error) failure("read approved deadline policy", result.error); return result.data;
    },
    async incidentRecord(enterpriseId: string, incidentId: string, table: "incident_responsibility_roles" | "incident_chronology_events" | "incident_evidence_snapshots" | "incident_impact_assessments" | "incident_regulatory_trigger_findings" | "incident_reviewer_decisions" | "incident_submission_packages" | "incident_external_submissions" | "incident_corrective_actions", id: string) {
      const result = await db.from(table).select("*").eq("enterprise_id", enterpriseId).eq("incident_id", incidentId).eq("id", id).maybeSingle();
      if (result.error) failure(`read ${table} record`, result.error); return result.data;
    },
    async bundle(enterpriseId: string, incidentId: string) {
      ...
      const results = await Promise.all(tables.map((table) => db.from(table).select("*").eq("enterprise_id", enterpriseId).eq(table === "incident_regulatory_assessments" ? "id" : "incident_id", incidentId).order("created_at", { ascending: true }).order("id", { ascending: true })));
