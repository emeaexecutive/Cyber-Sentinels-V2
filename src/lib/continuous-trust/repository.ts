...
export function continuousTrustRepository() {
  ...
  return {
    ...
    async alerts(enterpriseId: string, limit: number, status?: string | null) {
      let query = db.from("trust_alerts").select("id,enterprise_id,alert_type,status,subject_type,subject_reference,severity,detected_at,acknowledged_at,resolved_at,triggering_event_id,assessment_id,drift_id,policy_id,policy_version,evidence_references,remediation_guidance,assigned_to,updated_at,alert_title,alert_description").eq("enterprise_id", enterpriseId).order("detected_at", { ascending: false }).limit(limit);
      if (status) query = query.eq("status", status); const result = await query; return rows(result, "Continuous Trust alerts").map((row) => normalizeTrustAlert(row));
    },
    ...
    async replayAssessment(enterpriseId: string, decisionId: string) { const result = await db.from("continuous_trust_assessments").select("*,trust_drift_findings(*),trust_alerts(id,enterprise_id,alert_type,status,severity,detected_at,acknowledged_at,resolved_at,evidence_references,remediation_guidance,triggering_event_id,alert_title,alert_description)").eq("enterprise_id", enterpriseId).eq("state_decision_id", decisionId).maybeSingle(); if (result.error) fail("Continuous Trust Replay context", result.error); const data = result.data as Record<string, unknown> | null; if (data && Array.isArray((data as Record<string, unknown>).trust_alerts)) { (data as Record<string, unknown>).trust_alerts = ((data as Record<string, unknown>).trust_alerts as Record<string, unknown>[]).map((alert) => normalizeTrustAlert(alert)); } return data; },
