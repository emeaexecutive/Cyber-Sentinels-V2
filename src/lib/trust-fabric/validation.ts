...
export function validateTrustContract(value: unknown, enterpriseId: string): TrustContract {
  const row = strictObject(value, ["contractId","subject","workflow","authorizedObjective","requiredIdentityState","requiredAuthority","requiredEnvironmentState","permittedScope","permittedProviders","requiredEvidenceTypes","maximumEvidenceAgeSeconds","monitoringRequirements","humanReviewThresholds","contradictionPolicy","incidentThreshold","expiresAt","revokedAt","revocationState","issuer","approver","policyId","policyVersion","evidenceReferences","issuedAt","supersedesContractId","authorityScope","canDelegate","maximumDelegationDepth","authorityVersion"], "TrustContract");
  ...
  const revokedAt = row.revokedAt == null ? null : timestamp(row.revokedAt,"revokedAt");
  if (String(row.revocationState) === "revoked" && revokedAt === null) throw new TrustArchitectureApiError("revokedAt is required for a revoked Trust Contract.",400,"SCHEMA_INVALID");
  if (String(row.revocationState) === "active" && revokedAt !== null) throw new TrustArchitectureApiError("revokedAt must be null for an active Trust Contract.",400,"SCHEMA_INVALID");
  const contractIdentity = { enterpriseId, subjectType: subject.type, subjectId: subject.id, workflowId: workflow.id, issuedAt, policyId: row.policyId, policyVersion: row.policyVersion };
  ...
  const normalizedSubject = {type:subject.type as TrustContract["subject"]["type"],id:text(subject.id,"subject.id"),displayName:boundedText(subject.displayName,"subject.displayName",160)};
  const normalizedWorkflow = {id:text(workflow.id,"workflow.id"),objective:boundedText(workflow.objective,"workflow.objective",500)};
  let authorityScope: TrustContract["authorityScope"];
  if (row.authorityScope !== undefined) {
    const scope = strictObject(row.authorityScope, ["permittedActions","permittedTools","permittedTargets","environments","dataBoundary","financialLimit","executionLimit"], "authorityScope");
    if (!["PUBLIC","INTERNAL","CONFIDENTIAL","RESTRICTED"].includes(String(scope.dataBoundary))) throw new TrustArchitectureApiError("authorityScope.dataBoundary is invalid.",400,"SCHEMA_INVALID");
    const financialLimit = scope.financialLimit === null ? null : Number(scope.financialLimit);
    const executionLimit = scope.executionLimit === null ? null : Number(scope.executionLimit);
    if ((financialLimit !== null && (!Number.isSafeInteger(financialLimit) || financialLimit < 0)) || (executionLimit !== null && (!Number.isSafeInteger(executionLimit) || executionLimit < 0))) throw new TrustArchitectureApiError("authorityScope limits are invalid.",400,"SCHEMA_INVALID");
    authorityScope = { permittedActions:stringArray(scope.permittedActions,"authorityScope.permittedActions"), permittedTools:stringArray(scope.permittedTools,"authorityScope.permittedTools"), permittedTargets:stringArray(scope.permittedTargets,"authorityScope.permittedTargets"), environments:stringArray(scope.environments,"authorityScope.environments"), dataBoundary:String(scope.dataBoundary) as NonNullable<TrustContract["authorityScope"]>["dataBoundary"], financialLimit, executionLimit };
  }
  const canDelegate = row.canDelegate === undefined ? undefined : Boolean(row.canDelegate);
  const maximumDelegationDepth = row.maximumDelegationDepth === undefined ? undefined : Number(row.maximumDelegationDepth);
  if (maximumDelegationDepth !== undefined && (!Number.isSafeInteger(maximumDelegationDepth) || maximumDelegationDepth < 0 || maximumDelegationDepth > 16)) throw new TrustArchitectureApiError("maximumDelegationDepth is invalid.",400,"SCHEMA_INVALID");
  const authorityVersion = row.authorityVersion === undefined ? undefined : text(row.authorityVersion,"authorityVersion");
  return { contractId, enterpriseId, subject:normalizedSubject, subjectType:normalizedSubject.type, subjectId:normalizedSubject.id, workflow:normalizedWorkflow, workflowId:normalizedWorkflow.id, authorizedObjective:boundedText(row.authorizedObjective,"authorizedObjective",500), requiredIdentityState:row.requiredIdentityState as TrustContract["requiredIdentityState"], requiredAuthority:stringArray(row.requiredAuthority,"requiredAuthority"), requiredEnvironmentState:row.requiredEnvironmentState as TrustContract["requiredEnvironmentState"], permittedScope:stringArray(row.permittedScope,"permittedScope"), permittedProviders:stringArray(row.permittedProviders,"permittedProviders"), requiredEvidenceTypes:stringArray(row.requiredEvidenceTypes,"requiredEvidenceTypes"), maximumEvidenceAgeSeconds, monitoringRequirements:stringArray(row.monitoringRequirements,"monitoringRequirements"), humanReviewThresholds:stringArray(row.humanReviewThresholds,"humanReviewThresholds"), contradictionPolicy:row.contradictionPolicy as TrustContract["contradictionPolicy"], incidentThreshold:row.incidentThreshold as TrustContract["incidentThreshold"], expiresAt, revokedAt, revocationState:row.revocationState as TrustContract["revocationState"], issuer:text(row.issuer,"issuer"), approver:text(row.approver,"approver"), policyId:text(row.policyId,"policyId"), policyVersion:text(row.policyVersion,"policyVersion"), evidenceReferences, issuedAt, supersedesContractId, ...(authorityScope ? { authorityScope } : {}), ...(canDelegate === undefined ? {} : { canDelegate }), ...(maximumDelegationDepth === undefined ? {} : { maximumDelegationDepth }), ...(authorityVersion === undefined ? {} : { authorityVersion }) };
}
