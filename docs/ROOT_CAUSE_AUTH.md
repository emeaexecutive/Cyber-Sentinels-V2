...
- The product-proof branch Preview uses Cloudflare's official always-pass test pair and its documented `localhost` response hostname as branch-specific Vercel overrides. The overrides keep client token generation and server Siteverify validation active, apply to no other branch, and are ignored by Production code.
