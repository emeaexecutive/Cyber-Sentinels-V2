...
- A controlled Preview may use Cloudflare's official dummy pair only through explicit branch-scoped Vercel environment variables: `TURNSTILE_MODE=preview-test`, `NEXT_PUBLIC_TURNSTILE_SITE_KEY`, `TURNSTILE_SECRET_KEY`, and `TURNSTILE_EXPECTED_HOSTNAME`. The expected hostname is limited to Cloudflare's documented `localhost` dummy response or the previously observed `example.com` response. Code accepts that mode only when `VERCEL_ENV=preview`; Production, partial pairs, missing hostname configuration, and test credentials in live mode all fail closed. No credential is embedded in source.
