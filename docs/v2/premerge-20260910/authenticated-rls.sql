...
do $$ begin
 if (select count(*) from public.incident_evidence_links where incident_id='d509e2c4-f8f9-4cef-be03-bc3ece2068ca')<>8 then raise exception 'Owner authenticated read failed'; end if;
...
do $$ begin
 ...
 if exists(select 1 from public.incident_regulatory_assessments where id='d509e2c4-f8f9-4cef-be03-bc3ece2068ca') then raise exception 'Cross tenant incident read'; end if;
 if exists(select 1 from public.incident_submission_packages where incident_id='d509e2c4-f8f9-4cef-be03-bc3ece2068ca') then raise exception 'Cross tenant export read'; end if;
end $$;
reset role;
rollback;
