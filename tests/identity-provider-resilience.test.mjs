...
test("raw biometric and document payloads are never normalized", async () => { const result = await new StripeIdentityAdapter(client({})).collectSignal("IDENTITY_ASSERTION", context); const serialized = JSON.stringify(result); assert.equal(serialized.includes("raw_document"), false); assert.equal(serialized.includes("selfie"), false); assert.equal(serialized.includes("biometric_template"), false); });
test("World ID remains intact and new adapters are registered in the existing registry", async () => { const source = await readFile(new URL("../lib/identity-signals/adapters.ts", import.meta.url), "utf8"); assert.match(source, /WorldIdSafeAdapter/); assert.match(source, /new StripeIdentityAdapter\(\)/); assert.match(source, /new PersonaIdentityAdapter\(\)/); });
