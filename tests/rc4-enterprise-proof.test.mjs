...
test("homepage is action-led with four sections, two bounded CTAs and no blueprint visual", async () => {
  ...
  for (const outcome of ["Operational Trust Infrastructure", "ALLOW · REVIEW · DENY", "Every decision keeps the evidence needed for audit and Replay", "Trust is contextual to the action"]) {
    assert.match(source, new RegExp(outcome, "i"));
  }
});

test("demo presents one canonical fourteen-question cross-Epic Enterprise Trust journey", async () => {
  const [source,legacy] = await Promise.all([read("app/demo/page.tsx"),read("app/demo/trust-execution-flow/page.tsx")]);
  for(const question of ["Who or what acted?","What identity was verified?","What authority existed?","What environment was declared?","What environment was observed?","What scope was permitted?","What evidence supported the decision?","Why did trust change?","Was an incident opened?","What containment was requested?","What containment was confirmed?","Who reviewed it?","What corrective action followed?","How can the complete sequence be replayed?"]) assert.equal(source.includes(question),true);
