...
create table if not exists public.trust_domain_versions (
  ...
  supersedes_id uuid references public.trust_domain_versions(id) on delete restrict,
  change_event_id uuid references public.trust_events(id) on delete restrict,
...
drop policy if exists "authenticated reads active domain registry" on public.trust_domain_versions;
