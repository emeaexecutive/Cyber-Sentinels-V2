...
drop policy if exists "tenant reads protected workflows" on public.protected_workflows;
create policy "tenant reads protected workflows" on public.protected_workflows for select to authenticated
  using(public.user_can_access_trust_workspace(workspace_id));
drop policy if exists "tenant reads workflow interventions" on public.workflow_interventions;
create policy "tenant reads workflow interventions" on public.workflow_interventions for select to authenticated
  using(public.user_can_access_trust_workspace(workspace_id));
