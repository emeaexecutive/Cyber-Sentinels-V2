...
do $$ begin
 if (select count(*) from public.incident_evidence_links where incident_id='c355c3f6-814b-4aeb-821f-4fbac26f0f9b')<>8 then raise exception 'Owner authenticated read failed'; end if;
...
do $$ begin
 ...
 if exists(select 1 from public.incident_regulatory_assessments where id='c355c3f6-814b-4aeb-821f-4fbac26f0f9b') then raise exception 'Cross tenant incident read'; end if;
 if exists(select 1 from public.incident_submission_packages where incident_id='c355c3f6-814b-4aeb-821f-4fbac26f0f9b') then raise exception 'Cross tenant export read'; end if;
end $$;
reset role;
rollback;
