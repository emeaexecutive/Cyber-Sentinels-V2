...
const report = { target, consent: 'Browser-only rejected-optional fixture; no server consent claim', writes: 'All non-GET/HEAD requests blocked', widths: [], errors: [], console: [], hydrationScenarios: [] };
try {
  ...
  page.on('console', message => { if (['error', 'warning'].includes(message.type())) report.console.push({ level: message.type(), text: message.text().replace(/([?&]_vercel_share=)[^\s"&]+/g, '$1REDACTED') }); });
  ...
  for (const width of [1440, 1280, 1024, 768, 430, 390]) {
    ...
    for (const text of ['Approve €20,000 payment', 'VERIFIED', 'Maximum €10,000', 'Policy limit exceeded.', 'DENY', 'Identity is not authority.', 'The decision is only the beginning.', 'When something goes wrong, reconstruct what actually happened.', "Trust doesn't reset with each API call."]) assert.ok((await home.innerText()).includes(text), `Missing homepage requirement: ${text}`);
  ...
  await page.locator('header').getByRole('link', { name: 'Developers', exact: true }).click();
  await page.waitForURL('**/developers');
  await page.locator('main[aria-busy="true"]').waitFor({ state: 'detached', timeout: 90000 });
  report.developers = { path: new URL(page.url()).pathname, heading: await page.locator('h1').textContent() };
  await page.goto(target, { waitUntil: 'load' });
  ...
  assert.match(report.demo.heading, /demo/i);
  for (const width of [1440, 390]) {
    await page.setViewportSize({ width, height: 1000 });
    const session = await context.newCDPSession(page);
    await session.send('Network.enable');
    await session.send('Network.setCacheDisabled', { cacheDisabled: true });
    for (const mode of ['hard-reload', 'direct-url', 'navigation', 'slow-load']) {
      if (mode === 'slow-load') await session.send('Network.emulateNetworkConditions', { offline: false, latency: 150, downloadThroughput: 200000, uploadThroughput: 100000 });
      await page.goto(target, { waitUntil: 'load', timeout: 90000 });
      if (mode === 'hard-reload') await page.reload({ waitUntil: 'load', timeout: 90000 });
      if (mode === 'navigation') {
        await page.getByRole('link', { name: 'Explore the API', exact: true }).click();
        await page.waitForURL('**/developers/docs');
      ...
      report.hydrationScenarios.push({ width, mode, loggedOut: true, pageErrors: report.errors.length });
    }
    await session.send('Network.emulateNetworkConditions', { offline: false, latency: 0, downloadThroughput: -1, uploadThroughput: -1 });
    await session.detach();
  }
  report.hydrationMessages = report.console.filter(item => /hydration|hydrating|react error #(?:418|423|425)/i.test(item.text));
