...
try {
  ...
  if(mode==='create-isolation-workspace') {
    ...
    await writeFile(join(tmpdir(),'cyber-v1-control-plane-proof','isolation-workspace.json'),JSON.stringify({tenant,createdThrough:'Authenticated Create Workspace form',createdAt:new Date().toISOString()},null,2));
    ...
    for(const file of ['key-metadata.json','isolation-key-metadata.json']) {
      const metadata=JSON.parse(await readFile(join(tmpdir(),'cyber-v1-control-plane-proof',file),'utf8'));
      ...
      await writeFile('docs/release/v1-control-plane-production-proof/key-cleanup.json',JSON.stringify(cleanup,null,2));
    }
